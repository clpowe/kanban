...
describe("reward service", () => {
  ...
  test("redeems a reward when the child has enough points", async () => {
    ...
    await redeemReward(db as unknown as Database, childUser, reward.id);

    expect(updatedPoints as number | null).toBe(15);
  });
  ...
  test("does not redeem a reward when the child lacks enough points", async () => {
    ...
    await expect(
      redeemReward(db as unknown as Database, { ...childUser, points: 5 }, reward.id),
