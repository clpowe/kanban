...
async function apiFetch<T>(url: string, options?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
        ...
        throw new Error(errorJson?.error || errorText || `Request failed with status: ${res.statusText}`)
    }
...
export const api = {
    ...
    async switchActiveUser(userId: number): Promise<{ success: boolean; activeUserId: number }> {
        return apiFetch<{ success: boolean; activeUserId: number }>('/session/active-user', {
            method: 'PATCH',
    ...
    async createReward(reward: { title: string; cost: number }): Promise<RewardView> {
        return apiFetch<RewardView>('/api/rewards', {
            method: 'POST',
